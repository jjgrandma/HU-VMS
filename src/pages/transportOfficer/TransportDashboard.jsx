  ...
  const fetchData = async () => {
    try {
      const [requests, vehicles, drivers, complaints] = await Promise.all([
        getRequests(), getVehicles(), getDrivers(), getComplaints(),
      ]);
      ...
      complaints.filter(c => c.status !== 'Resolved').slice(0, 2).forEach(c => activities.push({
